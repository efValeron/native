...
let deepCopySuperUser = {...superUser, friends: superUser.friends.map(f => ({...f}))};
...
// "Donald"
let superUserCorrect2 = {...superUser, friend: superUser.friends.map(f => f.id === 2 ? {...f, name: "Donald"} : f)};
...
let superUserCorrect3 = {...superUser, friends: [...superUser.friends.map(f => ({...f})), {...newFriend}]};
...
for (let i = 1; i < students.length; i++) if (bestStudent.scores < students[i].scores) bestStudent = students[i]
// console.log(bestStudent)

//13. Найдите сумму баллов всех студентов (reduce)* ✔✔✔✔
let scoresSum = students.reduce((sum, s) => sum + s.scores, 0);
...
const addFriends = (students) => students.map(s => ({
  ...s,
  friends: students.filter(f => f.name !== s.name).map(f => f.name)
}))
...
const getBestStudents = (students, count = 1) => {
  const sortedStudents = [...students].sort((a, b) => b.scores - a.scores).slice(0, count)
  return [...sortedStudents, ...Array(count - sortedStudents.length).fill(null)]
}
